...
using OpenDesk.Infrastructure.ExceptionReporter;
using OpenDesk.Localization.Properties;
using OpenDesk.Presentation.Common.Commands;

namespace OpenDesk.Presentation.Common.ErrorReport
{
    class ErrorReportViewModel : ObservableObject
    {
        ...
        public void SubmitError()
        {
            ...
            string queryString = string.Format("from={0}&emaila={1}&file={2}",
                Uri.EscapeDataString("support@opendesk.com"),
                Uri.EscapeDataString("OpenDesk V3 Error Report"),
                Uri.EscapeDataString(tempFile));

            var c = new WebClient();
            var result = c.UploadFile("https://reports.opendesk.com/file.php?" + queryString, "POST", tempFile);
